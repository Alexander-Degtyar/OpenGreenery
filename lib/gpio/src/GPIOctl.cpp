...
GPIOctl::GPIOctl(const PinId _pin_id)
{
    ...
    auto wiring_cast = [_pin_id](const PinoutTable & _table) -> PinNumber
    {
            const auto wiring_pin = _table.find(_pin_id.pin);
            if (wiring_pin == _table.end())
            {
                throw std::domain_error("Unusable pin: "+toString(_pin_id.pinout)+"."+std::to_string(_pin_id.pin));
            }
            return wiring_pin->second;
    };

    switch (_pin_id.pinout)
    {
        ...
        {
            m_pin = wiring_cast(s_phys_to_wiring);
            break;
        }
        case Pinout::BROADCOM:
        {
            m_pin = wiring_cast(s_bcm_to_wiring);
            break;
        }
        default:
        {
